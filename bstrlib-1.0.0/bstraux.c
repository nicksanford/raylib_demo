 ...
 */
int bReplicate(bstring b, int n) { return bpattern(b, n * b->slen); }
 ...
 */
int bInsertChrs(bstring b, int pos, int len, unsigned char c,
                unsigned char fill) {
  if (b == NULL || b->slen < 0 || b->mlen < b->slen || pos < 0 || len <= 0)
    return -__LINE__;

  if (pos > b->slen && 0 > bsetstr(b, pos, NULL, fill))
    return -__LINE__;
...
int bJustifyCenter(bstring b, int width, int space) {
  ...
  if (0 > (ret = bJustifyLeft(b, space)))
    return ret;
  if (b->slen <= width)
    return bInsertChrs(b, 0, (width - b->slen + 1) >> 1, (unsigned char)space,
                       (unsigned char)space);
  return BSTR_OK;
}
...
static size_t bsUuDecodePart(void *buff, size_t elsize, size_t nelem,
                             void *parm) {
...
DecodeMore:;
  ...
  if (BSTR_ERR !=
      bsreada(luuCtx->io.src, luuCtx->sInp,
  ...
  for (i = 0; i < src->slen; i += UU_MAX_LINELEN) {
    ...
    for (j = i; j < jm; j += 3) {
      ...
      if (bconchar(out, UU_ENCODE_BYTE((c0 & 0xFC) >> 2)) < 0 ||
          bconchar(out, UU_ENCODE_BYTE(((c0 & 0x03) << 4) |
                                       ((c1 & 0xF0) >> 4))) < 0 ||
          bconchar(out, UU_ENCODE_BYTE(((c1 & 0x0F) << 2) |
                                       ((c2 & 0xC0) >> 6))) < 0 ||
